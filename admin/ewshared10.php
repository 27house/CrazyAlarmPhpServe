		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				...
				if ($item->GroupTitle && (!$this->IsRoot || !EW_MENU_ROOT_GROUP_TITLE_AS_SUBMENU)) { // Group title
					...
					if (strval($item->Text) <> "") {
						if ($this->IsMobile)
							$str .= "<li data-role=\"list-divider\">" .'<i class="'.$ico_arr_lxl[$i-0].'"></i>'. '<span class="menu-text">'.$item->Text .'</span>'. "</li>\n";
						else
							$str .= "<li class=\"dropdown-header\">" .'<i class="'.$ico_arr_lxl[$i-0].'"></i>'. '<span class="menu-text">'.$item->Text .'</span>'. "</li>\n";
					}
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
