...
class cuser_info extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('user_info', 'user_info', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// account
		$this->account = new cField('user_info', 'user_info', 'x_account', 'account', '`account`', '`account`', 200, -1, FALSE, '`account`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['account'] = &$this->account;

		// nickname
		$this->nickname = new cField('user_info', 'user_info', 'x_nickname', 'nickname', '`nickname`', '`nickname`', 200, -1, FALSE, '`nickname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nickname'] = &$this->nickname;

		// password
		$this->password = new cField('user_info', 'user_info', 'x_password', 'password', '`password`', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['password'] = &$this->password;

		// avatar
		$this->avatar = new cField('user_info', 'user_info', 'x_avatar', 'avatar', '`avatar`', '`avatar`', 201, -1, TRUE, '`avatar`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['avatar'] = &$this->avatar;

		// create_time
		$this->create_time = new cField('user_info', 'user_info', 'x_create_time', 'create_time', '`create_time`', 'DATE_FORMAT(`create_time`, \'%Y/%m/%d %H:%i:%s\')', 135, -1, FALSE, '`create_time`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['create_time'] = &$this->create_time;

		// login_time
		$this->login_time = new cField('user_info', 'user_info', 'x_login_time', 'login_time', '`login_time`', 'DATE_FORMAT(`login_time`, \'%Y/%m/%d %H:%i:%s\')', 135, -1, FALSE, '`login_time`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['login_time'] = &$this->login_time;

		// describe
		$this->describe = new cField('user_info', 'user_info', 'x_describe', 'describe', '`describe`', '`describe`', 201, -1, FALSE, '`describe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['describe'] = &$this->describe;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("user_infoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("user_infoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "user_infoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("user_infoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("user_infoadd.php", $this->UrlParm($parm));
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->account->Exportable) $Doc->ExportCaption($this->account);
				...
				if ($this->create_time->Exportable) $Doc->ExportCaption($this->create_time);
				if ($this->login_time->Exportable) $Doc->ExportCaption($this->login_time);
				if ($this->describe->Exportable) $Doc->ExportCaption($this->describe);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->account->Exportable) $Doc->ExportCaption($this->account);
				if ($this->nickname->Exportable) $Doc->ExportCaption($this->nickname);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->account->Exportable) $Doc->ExportField($this->account);
					...
					if ($this->create_time->Exportable) $Doc->ExportField($this->create_time);
					if ($this->login_time->Exportable) $Doc->ExportField($this->login_time);
					if ($this->describe->Exportable) $Doc->ExportField($this->describe);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->account->Exportable) $Doc->ExportField($this->account);
					if ($this->nickname->Exportable) $Doc->ExportField($this->nickname);
